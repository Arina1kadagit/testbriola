			...
			if ($http_code == 200) {
				...
				//Конец хлебных крошек
					...
					foreach ($arrayOfProducts as $index1 => $arrayOfProduct) {
						...
						//Проблема повторяющихся товаров
						//Решение: 
						//1) из товаров по заказу выводим только заказанные
						//2) из всех товаров по группе - толкьо не заказанные

						//
						if (($addClassInOrder == ' in_order')||($addClassInOrder == ' in_order hide_on_page')){
							//Выводим товары по матрице (полученной по ЗАКАЗУ)
						...
						echo '<label for="journalname"></label><input name="journalname" type="text" placeholder="введите количество" min="0" max="'.'0'.'" class="input" id="journalname" size="0" value="'.$productQuantity.'" oninput="sincMatrix(this)" onkeydown="onlyNumber(this)">';
						echo '</form>';
					...
					//+все товары по группе
					//Позиции по номенклатуре по выбранной группе

					$mysqlQText = "SELECT * FROM nomenclatures WHERE isdeleted =FALSE AND parentguid = '".$parentGUID."' ORDER BY guid";
					$mysqlQuery = mysqli_query($con, $mysqlQText);
					$mysqlErrNo = mysqli_errno($con);
					$mysqlError = mysqli_error($con);
				...
				//mysqli_close($con);
					...
					if ($numrows > 0) {
						...
						while ($row = mysqli_fetch_array($mysqlQuery, MYSQLI_ASSOC)) {
							...
							foreach ($arrayOfProducts as $index1 => $arrayOfProduct) {	
								if ($arrayOfProduct['productGUID'] == $row['guid']) {
									$productQuantity = $arrayOfProduct['productQuantity'];
									if ($productQuantity != 0) {
										$addClassInOrder = ' in_order';
									}
