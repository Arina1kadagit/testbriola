...
if ($textBodyResponse === false) {
	...
	//Выводить будем:
	//1) все товары из матрицы + заказанные не из матрицы
	//2)скрываем те, которые не относятся к выбранной категории

	//Запрос Аналогичный в showMatrix

	//Вывод всей номенклатуры 		
	//$mysqlQText = "SELECT guid, parentguid, shortname, expirationDate, measure FROM nomenclatures WHERE isdeleted = FALSE";
	$mysqlQText = "SELECT n.guid, parentguid, shortname, expirationDate, measure, number FROM nomenclatures n INNER JOIN nomenclaturesort s ON n.guid = s.guid WHERE isdeleted = FALSE ORDER BY number";
	$mysqlQuery = mysqli_query($con, $mysqlQText);
	$mysqlErrNo = mysqli_errno($con);
	$mysqlError = mysqli_error($con);
	...
	//Только для количества строк (порядок не важен)
	//$mysqlQText = "SELECT guid, parentguid, shortname, expirationDate, measure FROM nomenclatures WHERE isdeleted = FALSE AND parentguid = '".$_GET["parentGUID"]."'";
	$mysqlQText = "SELECT n.guid, parentguid, shortname, expirationDate, measure FROM nomenclatures n INNER JOIN nomenclaturesort s ON n.guid = s.guid WHERE isdeleted = FALSE AND parentguid = '".$_GET["parentGUID"]."'";
	$mysqlQuery = mysqli_query($con, $mysqlQText);
	$mysqlErrNo= mysqli_errno($con);
	$mysqlError = mysqli_error($con);
	...
	$mysqlQText = "SELECT n.guid, parentguid, shortname, expirationDate, measure, number FROM nomenclatures n INNER JOIN nomenclaturesort s ON n.guid = s.guid WHERE isdeleted = FALSE ORDER BY number";
	//$mysqlQText = "SELECT guid, parentguid, shortname, expirationDate, measure FROM nomenclatures WHERE isdeleted = FALSE ORDER BY parentNomenclaturName, fullname"
	$mysqlQuery = mysqli_query($con, $mysqlQText);
	$mysqlErrNo = mysqli_errno($con);
	$mysqlError = mysqli_error($con);
	...
	if ($numrows > 0) {
		...
		while ($row = mysqli_fetch_array($mysqlQuery, MYSQLI_ASSOC)) {
			...
			//Для товара в заказе изменяем кол-во - соответствие в матрице
			foreach ($_SESSION['session_userMatrix'] as $key => $arrProduct) {
			...
			//Соответствие для заказаннх товаров не по матрице
			if($_SESSION['session_userNoMatrix'] != ""){
			...
			if ($row['parentguid'] != $_GET["parentGUID"]) {
				...
				if (!($index >= $startIndex && $index <= $endIndex)) {
					$addClassInOrder.= ' all_in_category hide_on_page';
				}
				//для идентификации по категории для всего ассортимента
				else{
					$addClassInOrder.= ' all_in_category';
				}
				$index++;
			}
