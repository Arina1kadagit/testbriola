...
<?php
	...
	// Создать таблицу договоров, если её нет
	$mysqlQText = "CREATE TABLE IF NOT EXISTS `contracts` (
									`guid` varchar(36) NOT NULL,
									`fullname` varchar(1024) NOT NULL,
									`counterpartyguid` varchar(36) NOT NULL,
									`counterpartyname` varchar(1024) NOT NULL,
									`organizationguid` varchar(36) NOT NULL,
									`organizationname` varchar(1024) NOT NULL,
									`isdeleted` boolean NOT NULL,
									`seeonlyyourmatrix` boolean NOT NULL,
	...
	if ($mysqlErrNo != 0) $message.= 'Не удалось создать таблицу договоров, код ошибки: ' . $mysqlErrNo . ', текст ошибки: ' . $mysqlError . PHP_EOL . 'Текcт запроса: ' . PHP_EOL . $mysqlQText;
	...
	// Помещаем данные договора в таблицу
	$mysqlQText = "REPLACE INTO contracts (`guid`, `fullname`, `counterpartyguid`, `counterpartyname`, `organizationguid`, `organizationname`, `isdeleted`, `seeonlyyourmatrix`)
										VALUES ('".$data['GUID']."', '".$data['fullName']."', '".$data['counterpartyGUID']."', '".$data['counterpartyName']."', '".$data['organizationGUID']."', '".$data['organizationName']."', '".($data['isDeleted'] ? 1 : 0)."', '".($data['seeOnlyYourMatrix'] ? 1 : 0)."')";
	$mysqlQuery = mysqli_query($con, $mysqlQText);
	$mysqlErrNo = mysqli_errno($con);
	$mysqlError = mysqli_error($con);
	if ($mysqlErrNo != 0) $message.= 'Не удалось добавить или изменить договор, код ошибки: ' . $mysqlErrNo . ', текст ошибки: ' . $mysqlError . PHP_EOL . 'Тект запроса: ' . PHP_EOL . '"' . $mysqlQText . '"';
