...
$curl = curl_init();
curl_setopt($curl, CURLOPT_URL, BASE_1C_ADDRESS . '/hs/api/getMatrixWithOrder'); //из запроса берутся данные для шапки заказа
...
// Если получен любой ответ, даже с ошибкой
	...
	if ($http_code == 200) {
		if (gettype($arrayBodyResponse) == 'array') {
			...
			while ($row = mysqli_fetch_array($mysqlQuery, MYSQLI_ASSOC)) {
				...
				// вывод категории из второго уровня
				if ($numrows > 0) {
				//echo $mysqlQText;
	...
	//Вывод всей номенклатуры по матрице		
	...
	// Запрос только тех строк которые будут видны (чтобы предварительно знать их количество для вывода номеров страниц) - товары по категории
	//Важно толкьо кол-во страк - сортирвока не важна
	$mysqlQText = "SELECT guid, parentguid, shortname, expirationDate, measure FROM nomenclatures WHERE guid IN (".$strProductsGUIDs.") AND isdeleted =FALSE AND parentguid = '".$_GET["parentGUID"]."' ORDER BY guid";
	$mysqlQuery = mysqli_query($con, $mysqlQText);
	$mysqlErrNo= mysqli_errno($con);
	$mysqlError = mysqli_error($con);
	if ($mysqlErrNo2 != 0) $message.= 'Не удалось получить данные матрицы номенклатуры для указанной группы, код ошибки: ' . $mysqlErrNo . ', текст ошибки: ' . $mysqlError . PHP_EOL . 'Тект запроса: ' . PHP_EOL . $mysqlQText;
	$numrowsInGroup = mysqli_num_rows($mysqlQuery);
	$maxPage = ceil($numrowsInGroup/$rowsOnPage);
	//echo '</br>';
	//echo '2 - '.$numrowsInGroup;
	...
	////////////////////////////
	//Заказанные - не по матрице
	/*
	if($_SESSION['session_userNoMatrix'] != ""){
		foreach ($_SESSION['session_userNoMatrix'] as $key => $arrProduct){ 
			$productQuantity = $arrProduct['productQuantity']; //количество определяем из переменной сессии
			$productGUID = $arrProduct['productGUID'];
			if ($productQuantity != 0) {
				$addClassInOrder = ' in_order all_in_order';  //all_in_order - идентификатор категории товаров не из матрицы
			
			//ini_set('memory_limit', '-1');
			$mysqlQText2 = "SELECT * FROM nomenclatures WHERE isdeleted =FALSE AND guid = '".$productGUID."'";
			$mysqlQuery2 = mysqli_query($con, $mysqlQText2);
			$mysqlErrNo2 = mysqli_errno($con);
			$mysqlError2 = mysqli_error($con);
			
			if ($mysqlErrNo2 != 0) $message.= 'Не удалось получить данные матрицы номенклатуры, код ошибки: ' . $mysqlErrNo2 . ', текст ошибки: ' . $mysqlError2 . PHP_EOL . 'Тект запроса: ' . PHP_EOL . $mysqlQText2;
			$numrows2 = mysqli_num_rows($mysqlQuery2);

			$index = 0;

			if ($numrows2 > 0) {
				while ($row = mysqli_fetch_array($mysqlQuery2, MYSQLI_ASSOC)) {
					...
					//Из товаров категории выводит не заказанные
					...
					echo '<div id="product_measure">' . $row['measure'] . '</div>';
					echo '<form name="product_quantity" id="product_quantity" action="" method="post">';
					echo '<label for="journalname"></label><input name="journalname" type="text" placeholder="введите количество" min="0" max="'.'0'.'" class="input" id="journalname" size="0" value="'.$productQuantity.'" oninput="sincMatrix(this)" onkeydown="onlyNumber(this)" onkeypress="if(event.keyCode == 13) return false;">';
