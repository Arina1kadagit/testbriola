	...
	function Constellation (canvas, options) {
		var $canvas = $(canvas),
			...
			defaults = {
				...
				velocity: 0.1, 
				length: 1000,
				distance: 100, /*дистанция между точками*/
				radius: 150, /*расстояние между сцепляющимися точками*/
				stars: []
			},
		...
		function Star () {
			...
			this.vx = (config.velocity - (Math.random() * 0.5)); /*скорость*/
		...
		this.handlers = {
			window: {
				mousemove: function(e){
					config.position.x = e.pageX - $canvas.offset().left + 0; /*облась захвата курсора*/
					config.position.y = e.pageY - $canvas.offset().top + 350;
				},
				resize: function () {
